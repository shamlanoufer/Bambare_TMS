...
class AuthService {
  ...
  // ════════════════════════════════════════════════════════
  //  REGISTER — Step 1: Send OTP to verify phone number
  // ════════════════════════════════════════════════════════
  ...
  // ════════════════════════════════════════════════════════
  //  REGISTER — Step 2: Verify OTP → Create account → Save to Firestore
  //
  //  Flow:
  //  1. Verify phone OTP → get phoneCredential
  //  2. Create Email+Password account → get user
  //  3. Link phoneCredential to email account
  //     → now user can login with BOTH email AND phone
  //  4. Save ALL user data to Firestore
  // ════════════════════════════════════════════════════════
  Future<void> registerWithPhoneVerification({
    ...
    dynamic photoFile, // Can be File (mobile) or XFile (web)
  }) async {
    // Step 1: Create phone credential from OTP
    final phoneCredential = PhoneAuthProvider.credential(
      verificationId: verificationId,
      smsCode: otp.trim(),
    );

    // Step 2: Create the email+password Firebase account
    ...
    // Step 3: Link phone to this account
    // After this, user can login with email OR phone number
    try {
      await user.linkWithCredential(phoneCredential);
    } on FirebaseAuthException catch (e) {
      // If phone is already linked to another account, skip
    ...
    // Step 6: Save complete user data to Firestore
  ...
  // ── Helper: format phone number (Sri Lanka +94 E.164 for Firebase) ──
  static String formatPhone(String phone) {
    final trimmed = phone.trim();
    // Remove spaces, dashes; keep + for E.164
    final noSeparators = trimmed.replaceAll(RegExp(r'[\s\-]'), '');
    if (noSeparators.startsWith('+')) return noSeparators;
    final digitsOnly = noSeparators.replaceAll(RegExp(r'\D'), '');
