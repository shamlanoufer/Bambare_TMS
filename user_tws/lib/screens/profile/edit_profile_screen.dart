...
class _EditProfileScreenState extends State<EditProfileScreen> {
  ...
  ({String dial, String national}) _splitPhone(String raw) {
    final s = raw.trim();
    if (s.isEmpty) return (dial: '+94', national: '');
    final formatted = AuthService.formatPhone(s); // +[digits] (best effort)
    final digits = formatted.replaceAll(RegExp(r'\D'), '');
    if (digits.isEmpty) return (dial: '+94', national: '');

    // Prefer known dial codes; otherwise fall back to +94 and treat all digits as national.
    for (final dial in _phoneDials) {
    ...
    // If user stored local SL 07XXXXXXXX: keep +94 and show 7XXXXXXXX.
    if (digits.startsWith('07') && digits.length == 10) {
      return (dial: '+94', national: digits.substring(1));
    }
